...
class RegistroController
{
    ...
    public function registrarCuidador()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($resultado_usuario === true) {
                ...
                $resultado_perfil = $cuidador->registrarse();

                if ($resultado_perfil === true) {
                    header("Location: " . URL_BASE . "/login?exito=registrado");
                    exit();
                } elseif ($resultado_perfil === 'cedula_duplicada') {
                    $db->exec("DELETE FROM usuarios WHERE ID_Usuario = " . $id_usuario_nuevo);
                    $this->regresarConError("cedula_duplicada", $_POST);
                } else {
                    $db->exec("DELETE FROM usuarios WHERE ID_Usuario = " . $id_usuario_nuevo);
    ...
    private function regresarConError($error, $datos)
    {
        ...
        header("Location: " . URL_BASE . "/registro?error=" . $error);
        exit();
    }
}
